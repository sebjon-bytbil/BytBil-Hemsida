<?php
    function generateColor($color, $opacity) {
        $new_color = $color;
        $opacity = 100 - $opacity;
        if($opacity != 100 || $opacity == false) {
            $opacity = $opacity * 0.01;
            $new_color = hex2rgba($color, $opacity);
        }
        echo $new_color;
    }
?>
...
/* Wrapper */
<?php if (get_field("sitesettings-wrapper-width", $sid)) : ?>
    .wrapper {
    max-width: <?php the_field("sitesettings-wrapper-width", $sid); ?>px;
    }
<?php else : ?>
    .wrapper {
    max-width: 1170px;
...
<?php if (get_field('sitesettings-edit-background', $sid) == 'gradient') : ?>
    html {
    background: <?php the_field('sitesetting-background-gradient-end', $sid); ?>;
    }
...
<?php if (get_field('sitesetting-edit-type', $sid)) : ?>
    <?php
    $useCustom = get_field("sitesetting-font-family-header", $sid) == "annat";
    if ($useCustom) {
        $font = get_field("sitesetting-font-other-name", $sid);
    }

    if ($useCustom && in_array("paragraphs", get_field("sitesetting-font-other-locations", $sid))) {
        $pFont = $font;
    } else {
        $pFont = "'" . get_field("sitesetting-font-family-paragraph", $sid) . "', sans-serif;";
    }

    if ($useCustom && in_array("headers", get_field("sitesetting-font-other-locations", $sid))) {
        $hFont = $font;
    } else {
        $hFont = "'" . get_field("sitesetting-font-family-header", $sid) . "', sans-serif;";
    }
    ?>
    body, p {
    font-family: <?php echo $pFont; ?>;
...
<?php if (get_field('sitesetting-wrapper-edit', $sid)) : ?>
    ...
    <?php if (in_array('background', get_field('sitesetting-wrapper-edit', $sid))) : ?>
        background-color: <?php generateColor(get_field('sitesetting-wrapper-bgcolor', $sid), get_field('sitesetting-wrapper-opacity', $sid)); ?>;
    <?php endif; ?>
