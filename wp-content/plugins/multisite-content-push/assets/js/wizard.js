jQuery(document).ready(function ($) {
    ...
    // REFRESH CPTs
    $('#mcp-refresh-post-types').on('click', refresh_post_types);
    function refresh_post_types(e) {
        ...
        $('.spinner').show();
        $('#mcp-refresh-post-types').attr('disabled', true);

        $.ajax({
            url: ajaxurl,
            data: {
                blog_id: blog_id,
                action: 'mcp_get_blog_ajax_url'
            },
            type: 'post'
        }).done(function (data) {
            _ajaxurl = data;

            $('#mcp-cpt-list-wrap ul li').slideUp().remove();

            if (!_ajaxurl || _ajaxurl.trim() == '' || _ajaxurl == '0') {
                $('.spinner').hide();
                $('#mcp-refresh-post-types').attr('disabled', false);
                $('#mcp-cpt-list-wrap ul').append('<li>' + captions.blog_not_found + '</li>');
                return false;
            }

            var data = {
                action: 'mcp_retrieve_cpt_selectors_data',
                blog_id: $('#blog_id').val()
            };
            ...
            $.ajax({
                ...
                if (data !== '') {
                    $('#mcp-cpt-list-wrap ul').append(data);
                }
            })
                .always(function () {
                    $.ajax({
                        ...
                        type: 'post',
                        data: {
                            action: 'mcp_retrieve_cpt_custom_selector_data',
                            blog_id: $('#blog_id').val()
                        },
                    })
                        .done(function (data) {
    ...
    $('#mcp-posts-list #doaction').click(function (e) {
        e.preventDefault();
        ...
        $('.spinner').show();
        var data = {
            action: 'mcp_retrieve_single_post_data',
            post_ids: post_ids,
    ...
    $('input[name=users_selection]').change(function (element) {
        var value = $(this).val();
        if ('all' == value) {
            $('#mcp-users-list input[type=checkbox]').attr('disabled', true).attr('checked', false);
        }
        else {
            $('#mcp-users-list input[type=checkbox]').attr('disabled', false);
        }
    });

    $('#mcp-users-list #doaction').click(function (e) {
        e.preventDefault();
        ...
        $('.spinner').show();
        var data = {
            action: 'mcp_retrieve_single_user_data',
            user_ids: user_ids
        };
        $.ajax({
            url: ajaxurl,
    ...
    $('#add-blog').click(function (e) {
        ...
        if (!isNaN(blog_id) && blog_id !== 0) {
            var data = {
                action: 'mcp_retrieve_single_blog_data',
                blog_id: $('#blog_id').val()
            };
            $.ajax({
                url: ajaxurl,
    ...
    function update_blog_click_event() {
        $('.mcp-remove-blog').click(function (e) {
            e.preventDefault();
    ...
    function update_post_click_event() {
        $('.mcp-remove-post').click(function (e) {
            e.preventDefault();
    ...
    function update_user_click_event() {
        $('.mcp-remove-user').click(function (e) {
            e.preventDefault();
    ...
    function update_items_ids(item_id) {
        $.ajax({
            url: ajaxurl,
            type: 'post',
            data: {action: 'mcp_remove_item_id_from_list', item_id: item_id}
        });
    }
