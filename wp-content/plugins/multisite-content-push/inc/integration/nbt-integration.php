...
class MCP_NBT_Integrator
{
    ...
    public function init()
    {
        ...
        if ($settings['blog_templates_integration']) {
            add_action('blog_templates-push-after_pushing', array(&$this, 'add_relationship'), 10, 2);
    ...
    public function show_nbt_integration_notice()
    {
        $settings = mcp_get_settings();
        if (!$settings['blog_templates_integration']) {
            $link = add_query_arg('page', 'mcp_settings_page', network_admin_url('admin.php'));
            $dismiss_link = add_query_arg('dismiss_nbt_int_notice', 'true', network_admin_url());
            $message = sprintf(__('The New Blog Templates plugin is activated. You may want to <a href="%s">activate integration with Multisite Content Push</a> <a href="%s" class="button">Dismiss</a>', MULTISTE_CP_LANG_DOMAIN), $link, $dismiss_link);
            Multisite_Content_Push_Errors_Handler::show_error_notice($message);
        }
    }
