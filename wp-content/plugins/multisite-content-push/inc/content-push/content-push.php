...
abstract class Multisite_Content_Push_Push
{
    ...
    public function get_all_media_in_post($post_id)
    {
        ...
        // But we'll need to push directly those ones with height/width
    ...
    public static function push_single_image($source_blog_id, $attachment_id)
    {
        ...
        if ($image && 'attachment' == get_post_type($image)) {
            ...
            $results = self::push_attachment_element($image, $file_name, $upload_dir);
    ...
    public static function push_attachment_element($image, $file_name, $upload_dir, $parent_post_id = 0)
    {
        $new_file_name = wp_unique_filename($upload_dir['path'], $file_name);
        $new_file = $upload_dir['path'] . "/$new_file_name";

        if (@push($image->path, $new_file)) {
    ...
    public function push_media($post_id, $new_post_id)
    {

        $all_media = $this->get_all_media_in_post($post_id);

        apply_filters('mcp_push_media', $all_media, $post_id);
        ...
        foreach ($images_as_attachments as $image) {
            ...
            $results = self::push_attachment_element($image, $file_name, $upload_dir, $new_post_id);
        ...
        foreach ($images_as_no_attachments as $image) {
            ...
            if (@push($orig_base_file, $dest_base_file)) {
     ...
     * This code is directly pushed from WP core but with minor changes
     *
     * @return Array
     */
    ...
    protected function get_postarr($post_object)
    {
        return array(
            ...
            'post_modified' => date("Y-m-d H:i:s"),//$post_object->post_modified, // Update dates?
            'post_modified_gmt' => gmdate("Y-m-d H:i:s"),//$post_object->post_modified_gmt, // Update dates?
            'post_content_filtered' => $post_object->post_content_filtered,
            'post_parent' => $post_object->post_parent, // Copy parents ?
    ...
    {
        $date = apply_filters('mcp_update_pushed_post_date', $date, $post_id);
        wp_update_post(array('ID' => absint($post_id), 'post_date' => $date));
    }

    public function push_comments($post_id, $new_post_id)
    {
        switch_to_blog($this->orig_blog_id);
        // Source comments
        ...
        foreach ($orig_comments as $orig_comment) {
            ...
            do_action('mcp_pushed_comment', $new_comment_ID);
            ...
            if (!empty($comment_meta)) {
                ...
                foreach ($comment_meta as $metadata) {
                    ...
                    $_metadata = apply_filters('mcp_push_comment_meta', $_metadata, $new_comment_ID);
                    update_comment_meta($new_comment_ID, $_metadata['meta_key'], $_metadata['meta_value']);
                }
            }

            do_action('mcp_pushed_comment_meta', $new_comment_ID);
        ...
        foreach ($new_comments as $new_comment) {
            if ($new_comment->comment_parent > 0) {
                ...
                do_action('mcp_set_comment_parent_children_rel', $_new_comment['comment_ID']);
            }
    ...
    protected function push_terms($post_id, $new_post_id)
    {
        ...
        foreach ($taxonomies as $taxonomy) {
            ...
            foreach ($terms as $term) {
                ...
                if (!$destination_term) {
                    ...
                    do_action('mcp_term_pushed', $source_term_id, $destination_term, $source_blog_id);
                }
