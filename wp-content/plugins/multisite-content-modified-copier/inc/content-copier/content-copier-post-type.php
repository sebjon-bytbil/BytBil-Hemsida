...
//            update_post_meta($item_id, 'mcc_copied', true, $this->posts_created[$item_id]);
            ...
            $post_created = $this->copy_item($item_id);
            $new_post_id = intval($post_created["new_post_id"]);

            update_post_meta($new_post_id, 'mcc_copied', true, $post_created);

            update_post_meta($new_post_id, 'mcc_orig_id', $item_id);
...
//                do_action('mcc_copy_post_meta', $this->orig_blog_id, $item_id, $new_item_id, $post_meta->meta_key, $unserialized_meta_value);
                foreach ($fields as $obj) {
                    if ($obj && $obj["type"] == "image") {
                        maybe_handle_image($obj, $new_item_id, $orig_post_meta);
                    } else if ($obj && $obj["type"] == "relationship") {
                        maybe_handle_relationship($obj, $new_item_id);
                    } else if ($obj && ($obj["type"] == "post_object" || $obj["type"] == "page_link")) {
                        maybe_handle_page_ref($obj, $orig_post_meta, $new_item_id);
    ...
    protected function get_postarr($post_object)
    {
        return array(
            ...
            'post_parent' => !empty($post_object->post_parent) ? $post_object->post_parent : 0, // Copy parents ?
    ...
    public function maybe_handle_relationship($obj, $new_post_id)
    {
    ...
    public function maybe_handle_page_ref($obj, $orig_post_meta, $new_post_id)
    {
    ...
    public function maybe_handle_image($obj, $new_post_id, $orig_post_meta)
    {
    ...
    public function maybe_resolve_missing($post_id)
    {
        ...
        //Bytbil, detta kommer inte att fungera med nya versionen av content-copier...
