<?php
/*
Plugin Name: Multisite Content Copier - Bytbil Modified
Plugin URI: https://premium.wpmudev.org/project/multisite-content-copier/
Description: Copy any content from any site in your network to any other site or group of sites in the same network. Modified version by bytbil
Author: WPMU DEV / Leo/Eric - Bytbil
Version: 1.4.1
...
Network:true
WDP ID: 855336
Tags: multisite plugin, multisite, bytbil
*/
...
class Multisite_Content_Copier
{
    ...
    public function display_not_multisite_notice()
    {
        ?>
        <div class="error">
            <p><?php _e('Multisite Content Copier is a plugin just for multisites, please deactivate it.', MULTISTE_CC_LANG_DOMAIN); ?></p>
        </div>
    <?php
    }
    ...
    public function enqueue_styles()
    {
        ...
        if (version_compare($wp_version, '3.8', '>=')) {
            ?>
            <style>
                #adminmenu #toplevel_page_mcc_network_page div.wp-menu-image:before {
                    content: "\f325";
                }
            </style>
        <?php
    ...
    {
        $url = preg_replace("/.*(\/wp-content.*)/", "$1", dirname(__FILE__) . "/");
        // Basics
        define('MULTISTE_CC_VERSION', '1.4');
        define('MULTISTE_CC_PLUGIN_URL', $url);
        define('MULTISTE_CC_PLUGIN_DIR', dirname(__FILE__) . "/");
        define('MULTISTE_CC_PLUGIN_FILE_DIR', dirname(__FILE__) . '/multisite-content-copier.php');
