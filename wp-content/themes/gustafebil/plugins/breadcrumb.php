<?php
function the_breadcrumb($string = false)
{
        ...
        if (is_category()) {
            ...
            if (get_post_type() != 'post') {
                if(get_post_type() == 'offer'){
                    $archive_page = get_field('settings-offers-page', 'options');
                    $post_type = get_post_type_object(get_post_type());
                    $slug = $post_type->rewrite;
                    echo '<a href="' . $archive_page . '">' . $post_type->labels->name . '</a>';
                    if ($showCurrent == 1) echo ' ' . $delimiter . ' ' . $before . get_the_title() . $after;
                }
                elseif(get_post_type() == 'news'){
                    $archive_page = get_field('settings-news-page', 'options');
                    $post_type = get_post_type_object(get_post_type());
                    $slug = $post_type->rewrite;
                    echo '<a href="' . $archive_page . '">' . $post_type->labels->name . '</a>';
                    if ($showCurrent == 1) echo ' ' . $delimiter . ' ' . $before . get_the_title() . $after;
            ...
            echo $before . 'Sidan hittades inte' . $after;
        }
