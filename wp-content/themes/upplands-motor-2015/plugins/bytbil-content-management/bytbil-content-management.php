...
function bytbil_content_management_styles()
{
    ...
    //if ($post_type == 'page' || $post_type == 'slideshow') {
...
function elasticaccess_scripts($hook)
{
    ...
    if ( $hook != 'post.php' && $hook != 'toplevel_page_acf-options-installningar' ) {
        return;
    }


    //wp_enqueue_script('elasticaccess-admin', '//code.jquery.com/jquery-latest.min.js', array(), '1.0.0', true);
    wp_enqueue_script('elasticaccess-admin', get_stylesheet_directory_uri() . '/js/ElasticAccess.js', array(), '1.0.0', true);
//    wp_enqueue_script('elasticaccess-admin-extra', get_stylesheet_directory_uri() . '/js/ElasticAccessAdmin.js', array(), '1.0.0', true);

    wp_register_style('bootstrap_style', get_stylesheet_directory_uri() . '/lib/bootstrap-3.3.5/less/bootstrap-elasticaccess.css', false, '1.0.0');
    wp_enqueue_style('bootstrap_style');

    wp_register_style('elasticaccess_ap_style', get_stylesheet_directory_uri() . '/css/elasticaccess-admin-style.css', false, '1.0.0');
...
function elasticaccess_scripts_frontend($hook){
    ...
    wp_register_script( 'elasticaccess-front', get_stylesheet_directory_uri() . "/js/ElasticAccessFront.js", '1.0.0', true );
