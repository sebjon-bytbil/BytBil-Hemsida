...
function get_employee_list($cities, $department, $vehicle_type=false, $brand, $hide, $show_vehicle_type, $employee_args=false)
//function get_employee_list($cities, $department, $brand, $hide, $employee_args=false)

{
    if ($brand && $cities) {

        $employee_cities_keys = $cities;
    ...
    foreach ($employees as $employee) {
        $hit = false;
        $type_hit = false;
        $empl_dep = get_field('employee-department', $employee->ID, true);
        $empl_vehicle_type = get_field('employee-vehicle_type', $employee->ID);

        if(!$empl_vehicle_type){
            $type_hit = true;
        }else{
            foreach ($vehicle_type as $vt) {
                foreach ($empl_vehicle_type as $e_vt) {
                    if ($vt == $e_vt) {
                        $type_hit = true;
                    }
                }
        ...
        if ($hit && $type_hit) {
            get_employee_card($employee->ID, $hide, $show_vehicle_type);
        }
    }
...
function get_employee_card($id, $hide, $show_vehicle_type=false)
{
    $image = get_field('employee-image', $id);
    if ($image) {
        $image_url = $image['url'];
        $image_url = maybe_add_preview_to_url($image_url);
    } else {
    ...
    $employee_vehicle_types = get_field('employee-vehicle_type', $id);
    $employee_car = "";
    $employee_transport = "";

    foreach($employee_vehicle_types as $type) {
        if($type === "car"){
            $employee_car = "Personbil";
        }
        if($type === "transport"){
            $employee_transport = "Transportbil";
        }
    }
    ...
    <div class="col-xs-12 col-sm-3">
        <div class="employee-card" id="employeee-<?php echo $id; ?>">
            <div class="employee-image">
                <img class="img-responsive img-100" src="<?php echo $image_url; ?>"/>
            ...
            <div class="employee-information block white-bg" data-hide="<?php echo $hide; ?>">
                ...
                <?php if(!$show_vehicle_type){ ?>
                    <span class="employee-vehicletype">
                        <?php
                        if($employee_car && $employee_transport){
                            echo $employee_car . " & " . $employee_transport;
                        }else if($employee_transport){
                            echo $employee_transport;
                        }else if($employee_car){
                            echo $employee_car;
                        }
                        ?>
                    </span>
