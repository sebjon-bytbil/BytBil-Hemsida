...
class Multisite_Content_Copier_Network_Main_Menu extends Multisite_Content_Copier_Admin_Page
{
    ...
    public function init_wizard()
    {
        ...
        if (isset($_GET['mcc_action']) && 'mcc_submit_metabox' == $_GET['mcc_action'] && wp_verify_nonce($_GET['_wpnonce'], 'mcc_submit_meta_box')) {
            ...
            if (!in_array($_GET['dest_blog_type'], array('group', 'all', 'nbt_group', 'specific')))
                return false;
            ...
            if ($dest_blog_type == 'group' && !$model->is_group(absint($_GET['group']))) {
                ...
                $this->wizard->set_value('dest_blogs_ids', $ids);
            } elseif ($dest_blog_type == "specific") {
                $blogs = $_GET["dest_blogs"];
                $ids = explode(",", $blogs);
            ...
            if ($redirect = wp_get_referer()) {
                ...
                $this->wizard->set_value("redirect_to", $_SERVER["HTTP_REFERER"]);
            }
...
<?php
    ...
    private function render_step_1()
    {
        ...
        <form action="" method="post" name="wizardform" id="wizardform">
            ...
            <ul class="wizardoptions">
                <li><label><input type="radio" name="mcc_action"
                                  value="add-page" <?php checked($current_action == 'add-page' || empty($current_action)); ?>> <?php _e('Pages', MULTISTE_CC_LANG_DOMAIN); ?>
                    </label></li>
                <li><label><input type="radio" name="mcc_action"
                                  value="add-post" <?php checked($current_action == 'add-post'); ?>> <?php _e('Posts', MULTISTE_CC_LANG_DOMAIN); ?>
                    </label></li>
                <li><label><input type="radio" name="mcc_action"
                                  value="add-cpt" <?php checked($current_action == 'add-cpt'); ?>> <?php _e('Custom Post Type (products, events...)', MULTISTE_CC_LANG_DOMAIN); ?>
                    </label></li>
                <li><label><input type="radio" name="mcc_action"
                                  value="add-user" <?php checked($current_action == 'add-user'); ?>> <?php _e('Users', MULTISTE_CC_LANG_DOMAIN); ?>
                    </label></li>
                <li><label><input type="radio" name="mcc_action"
                                  value="activate-plugin" <?php checked($current_action == 'activate-plugin'); ?>> <?php _e('Activate plugins', MULTISTE_CC_LANG_DOMAIN); ?>
                    </label></li>
            </ul>
            <p class="submit">
                <input type="submit" name="submit_step_1" class="button button-primary button-hero alignleft"
                       value="<?php _e('Next Step &raquo;', MULTISTE_CC_LANG_DOMAIN); ?>">
            </p>
        </form>
    <?php
    }
    ...
    private function render_blog_selector($current_action)
    {
        ...
        <h3><?php _e('Enter the id or URL of the site containing the pages you wish to copy', MULTISTE_CC_LANG_DOMAIN); ?></h3>
        <br/>
    <?php elseif ('add-post' == $current_action): ?>
        <h3><?php _e('Enter the id or URL of the site containing the post you wish to copy', MULTISTE_CC_LANG_DOMAIN); ?></h3>
        <br/>
    <?php
    elseif ('add-cpt' == $current_action): ?>
        <h3><?php _e('Enter the id or URL of the site containing the custom post types you wish to copy', MULTISTE_CC_LANG_DOMAIN); ?></h3>
        <br/>
    <?php
    elseif ('add-user' == $current_action): ?>
        <h3><?php _e('Enter the id or URL of the site containing the users you wish to copy', MULTISTE_CC_LANG_DOMAIN); ?></h3>
        <br/>
    <?php endif; ?>
        <input name="blog_id" type="text" id="blog_id" size="6" value="<?php echo $content_blog_id; ?>"
               placeholder="<?php _e('Blog ID', MULTISTE_CC_LANG_DOMAIN); ?>"/>
        ...
        <div style="display:inline-block" class="ui-widget">
            <label for="search_for_blog"> <?php _e('Or search by site URL', MULTISTE_CC_LANG_DOMAIN); ?>
                ...
                <span
                    class="description"><?php _e('Enter either the subdomain or the sub-folder, ie. for mysite.mydomain.com or mydomain.com/mysite, type "mysite"', MULTISTE_CC_LANG_DOMAIN); ?></span>
            </label>
        </div>
    ...
    private function render_post_selector($blog_id, $type)
    {
        ...
        <h3><?php _e('Select the items that you would like to push.', MULTISTE_CC_LANG_DOMAIN); ?></h3>

        <ul id="posts-list">
            <?php //echo $posts_list; ?>
        </ul>

        <input type="checkbox" name="settings[copy_images]" checked="true" style="display: none;">
        ...
        <div class="alignright" id="mcc-posts-list" style="width:100%;">
    ...
    <div class="alignleft" style="width:35%;">
        ...
        <label>
            <input type="radio" name="users_selection" <?php checked($user_selection == 'all'); ?>
                   value="all"/> <?php _e('All users', MULTISTE_CC_LANG_DOMAIN); ?>
        </label><br/><br/>

        <label>
            <input type="radio" name="users_selection" <?php checked($user_selection == 'ids'); ?>
                   value="ids"> <?php _e('Selected users', MULTISTE_CC_LANG_DOMAIN); ?>
        </label>
    </div>
...
<?php
        ...
        <form action="" method="post">
            ...
            <div id="blogs-list-wrap">
                <input name="blog_id" type="text" id="blog_id" size="6" style="float: left; margin-right: 10px;"
                       placeholder="<?php _e('Site ID', MULTISTE_CC_LANG_DOMAIN); ?>"/>

                <div style="display:inline-block" class="ui-widget">
                    <label for="search_for_blog"> <?php _e('Or search by site URL', MULTISTE_CC_LANG_DOMAIN); ?>
                        <input type="text" id="autocomplete" data-type="sites" class="medium-text">
                        <span class="spinner"></span> <input type="button" class="button button-primary" name="add-blog"
                                                             id="add-blog"
                                                             value="<?php _e('Add Site', MULTISTE_CC_LANG_DOMAIN); ?>"></input>
                    </label>
                </div>
                <div class="clear"></div>
                <div id="blogs-list">
    ...
    public function get_row_list($slug, $id, $title)
    {
        ...
        <li id="<?php echo $slug; ?>-<?php echo $id; ?>">
            <span class="remove-box"><a class="mcc-remove-<?php echo $slug; ?>" href=""
    ...
    public function get_row_cpt_selector_list($id, $title, $selected = false, $custom = false)
    {
        ...
        <li id="cpt-<?php echo $id; ?>">
            <?php if (!$custom): ?>
                <label><input type="radio" name="mcc_cpt"
            ...
            <?php else: ?>
                <label for="mcc_cpt_custom">
                    ...
                    <?php echo $title; ?><br/>
                    <span
                        class="description"><?php _e('Or insert the slug of your Custom Post Type here if you don\'t see it in the list', MULTISTE_CC_LANG_DOMAIN); ?></span>
                </label>
                <script>
                    jQuery(document).ready(function ($) {
    ...
    public function render_step_5()
    {
        ...
        if ('all' != $dest_blog_type) {
            ...
            <script>
                window.location.href = "<?php echo $this->wizard->get_value("redirect_to"); ?>";
            </script>
            <?php
    ...
    public function render_step_6()
    {
        ...
        <?php if ($this->wizard->get_value("redirect_to")) : ?>
        <script>
            window.location.href = "<?php echo $this->wizard->get_value("redirect_to"); ?>";
        </script>
    <?php endif;
