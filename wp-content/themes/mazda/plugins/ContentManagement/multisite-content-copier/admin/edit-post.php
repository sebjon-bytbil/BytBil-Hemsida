...
//		    );
    ...
    public function render_copier_meta_box($post)
    {
        ...
        if (!in_array($post->post_status, array('publish', 'draft', 'pending'))) {
            ...
            <?php $this->render_form_push_fields(); ?>
    ...
    /**
     * Render the Content Push form
     * Added by Erc
     */
    public function render_form_push_fields()
    {
        $sites = wp_get_sites(); ?>
    ...
    public function render_syncer_meta_box()
    {
        ...
        ?>
        <input type="submit" class="button-primary" name="mcc_sync_submit"
               value="<?php _e('Sync content', MULTISTE_CC_LANG_DOMAIN); ?>"/>
        <?php
    ...
    private function render_form_fields($prefix_slug)
    {
        ...
        <div style="margin-left:20px;">
            <p>
                <label>
                    <input type="radio" name="<?php echo $prefix_slug; ?>dest_blog_type"
                           class="<?php echo $prefix_slug; ?>dest_blog_type" value="all">
            ...
            <p>
                <label>
                    <input type="radio" name="<?php echo $prefix_slug; ?>dest_blog_type"
                           class="<?php echo $prefix_slug; ?>dest_blog_type" value="group">
            ...
            <?php if ($settings['blog_templates_integration']): ?>
                <p>
                    <label>
                        <input type="radio" name="<?php echo $prefix_slug; ?>dest_blog_type"
                               class="<?php echo $prefix_slug; ?>dest_blog_type" value="nbt-group">
                        <?php _e('Blog Templates Group', MULTISTE_CC_LANG_DOMAIN); ?>
                    </label>
                    <select name="<?php echo $prefix_slug; ?>nbt_blog_group"
                            id="<?php echo $prefix_slug; ?>nbt_blog_group">
                        <?php mcc_get_nbt_groups_dropdown(); ?>
                    </select>
                </p>
            <?php endif; ?>
        ...
        <ul style="margin-left:20px;">
            <?php foreach ($options as $option_slug => $label): ?>
                <li><label><input type="checkbox" class="mcc_options" name="<?php echo $option_slug; ?>"
                                  value="<?php echo $option_slug; ?>"></input> <?php echo $label; ?></label></li>
            <?php endforeach; ?>
        </ul>
    <?php
    }
