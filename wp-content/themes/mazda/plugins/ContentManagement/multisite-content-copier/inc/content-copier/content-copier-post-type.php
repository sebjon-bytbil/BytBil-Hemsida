...
class Multisite_Content_Copier_Post_Type_Copier extends Multisite_Content_Copier_Abstract
{
    ...
    public function copy_post($item_id)
    {
        ...
        $postarr["post_parent"] = $this->maybe_push_parent($postarr["post_parent"], $this);
        ...
        update_post_meta($new_item_id, 'mcc_orig_id', $orig_post->ID);
        ...
        if ($new_item_id) {
            ...
            foreach ($orig_post_meta as $post_meta) {
                ...
                // Is it acf?
                if (!is_array($unserialized_meta_value)) {
                    if (substr($unserialized_meta_value, 0, 6) == "field_") {
                        $obj = get_field_object($unserialized_meta_value, $new_item_id);
                        $fields[] = $obj;
                    } else {
            ...
            foreach ($fields as $obj) {
                if ($obj && $obj["type"] == "image") {
                    $this->maybe_handle_image($obj, $new_item_id, $orig_post_meta);
                } else if ($obj && $obj["type"] == "relationship") {
                    $this->maybe_handle_relationship($obj, $new_item_id);
                } else if ($obj && ($obj["type"] == "post_object" || $obj["type"] == "page_link")) {
                    $this->maybe_handle_page_ref($obj, $orig_post_meta, $new_item_id);
                } else if ($obj && $obj["type"] == "repeater") {
                    $obj = get_field_object($obj["key"], $new_item_id, true);
                    $this->maybe_link_repeater($obj, $orig_post_meta, array(
                        "new_item_id" => $new_item_id,
                        "orig_post" => $orig_post
                    ));
                } else {
                    update_post_meta($new_item_id, $item_meta->meta_key, $unserialized_meta_value);
                    update_post_meta($new_item_id, "_" . $obj["name"], $obj["key"]);
                }
            }
    ...
    protected function set_postarr($post_object, $postarr)
    {
        $post_object->menu_order = $postarr['menu_order'];
        $post_object->comment_status = $postarr['comment_status'];
        $post_object->ping_status = $postarr['ping_status'];
        $post_object->post_author = $postarr['post_author'];
        $post_object->post_date = $postarr['post_date'];
        $post_object->post_date_gmt = $postarr['post_date_gmt'];
        $post_object->post_content = $postarr['post_content'];
        $post_object->post_title = $postarr['post_title'];
        $post_object->post_name = $postarr['post_name'];
        $post_object->post_excerpt = $postarr['post_excerpt'];
        $post_object->post_status = $postarr['post_status'];
        $post_object->post_password = $postarr['post_password'];
        $post_object->to_ping = $postarr['to_ping'];
        $post_object->pinged = $postarr['pinged'];
        $post_object->post_modified = $postarr['post_modified'];
        $post_object->post_modified_gmt = $postarr['post_modified_gmt'];
        $post_object->post_content_filtered = $postarr['post_content_filtered'];
        $post_object->post_type = $postarr['post_type'];
        $post_object->post_mime_type = $postarr['post_mime_type'];
        $post_object->filter = $postarr['filter'];
        $post_object->format_content = $postarr['format_content'];

        return $post_object;
    }

    public function maybe_push_parent($parent_id, $class)
    {
        if ($parent_id == 0 || !$parent_id) {
            return 0;
        }

        $lpid = $this->maybe_get_local_id($parent_id);
    ...
    public function maybe_link_repeater($obj, $orig_post_meta, $params)
    {
        $new_item_id = $params["new_item_id"];
        $orig_post = $params["orig_post"];
        $repeater_rows = intval(get_post_meta($new_item_id, $obj["name"], true));
        update_post_meta($new_item_id, "_" . $obj["name"], $obj["key"]);
        ...
        for ($i = 0; $i < $repeater_rows; $i++) {
            foreach ($obj["sub_fields"] as $field) {
                ...
                if ($field["type"] == "image") {
                    $this->maybe_handle_image($field, $new_item_id, $orig_post_meta);
                } else if ($field["type"] == "repeater") {
                    $this->maybe_link_repeater($field, $orig_post_meta, $params);
                } else if ($field["type"] == "page_link" || $field["type"] == "post_object") {
                    $this->maybe_handle_page_ref($field, $orig_post_meta, $new_item_id);
                } else if ($field["type"] == "relationship") {
                    $this->maybe_handle_relationship($field, $new_item_id);
                } else {
                    update_post_meta($new_item_id, "_" . $new_meta_key, $field["key"]);
                }
            }
        }
    }

    public function maybe_handle_image($obj, $new_item_id, $orig_post_meta)
    {
        ...
        if (!!$iid) {
            ...
            update_post_meta($new_item_id, $obj["name"], intval($id));
            update_post_meta($new_item_id, "_" . $obj["name"], $obj["key"]);
            return $id;
        }
    }

    public function maybe_handle_relationship($obj, $new_item_id)
    {
        $data = get_post_meta($new_item_id, $obj["name"], true);
        ...
        if (is_array($data)) {
            ...
            foreach ($data as $pid) {
                ...
                if ($lpid == 0) {
                    $lpid = $this->maybe_resolve_missing($pid);
                }

                $newData[] = "" . $lpid;
            }
            update_post_meta($new_item_id, $obj["name"], $newData);
            update_post_meta($new_item_id, "_" . $obj["name"], $obj["key"]);
            return $newData;
        }
    }

    public function maybe_handle_page_ref($obj, $orig_post_meta, $new_item_id)
    {
        foreach ($orig_post_meta as $meta) {
            if ($meta->meta_key == $obj["name"]) {
                ...
                if ($lpid == 0) {
                    $lpid = $this->maybe_resolve_missing($meta->meta_value);
                }

                update_post_meta($new_item_id, $obj["name"], $lpid);
                update_post_meta($new_item_id, "_" . $obj["name"], $obj["key"]);
                return $lpid;
            }
        }
    ...
    public function maybe_resolve_missing($post_id)
    {
        $lpid = 0;
        ...
        if ($orig_post->post_type == "page") {
            ...
            $copier = new Multisite_Content_Copier_Post_Type_Copier(get_master_site_id(), array());
        }

        $new_post = $copier->copy_item($post_id);
