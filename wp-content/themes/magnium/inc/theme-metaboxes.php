...
function magnium_pages_settings_box() {
    ...
    foreach ( $screens as $screen ) {
        ...
        add_meta_box(
            'magnium_pages_settings_box',
            __( 'Page settings', 'magnium' ),
            'magnium_pages_settings_inner_box',
            $screen,
            'normal',
            'high'
        );
...
add_action( 'add_meta_boxes', 'magnium_pages_settings_box' );

function magnium_pages_settings_inner_box( $post ) {

  wp_enqueue_script('wp-color-picker');
  wp_enqueue_style( 'wp-color-picker' );

  // Add an nonce field so we can check for it later.
  wp_nonce_field( 'magnium_pages_settings_inner_box', 'magnium_pages_settings_inner_box_nonce' );
  ...
  echo '<p><input type="checkbox" id="page_fullwidth" name="page_fullwidth" '.$checked.' /> <label for="page_fullwidth">'.__( "Allow fullwidth elements on this page (rows with 'Fullwidth row' option checked will be fullwidth, assigned page sidebar will be disabled)", 'magnium' ).'</label></p>';

  $checked = '';
  if( $value_page_stick_footer == true ) { 
    $checked = 'checked = "checked"';
  }
  echo '<p><input type="checkbox" id="page_stick_footer" name="page_stick_footer" '.$checked.' /> <label for="page_stick_footer">'.__( "Stick page content to footer (disable page bottom margin)", 'magnium' ).'</label></p>';
  ...
  echo '<p><input type="checkbox" id="page_notdisplaytitle" name="page_notdisplaytitle" '.$checked.' /> <label for="page_notdisplaytitle">'.__( "Don't display this page title (only show page content)", 'magnium' ).'</label></p>';
  ...
  echo '<p><label for="page_sidebarposition" style="display: inline-block; width: 150px;">'.__( "Page sidebar position: ", 'magnium' ).'</label>';
  echo '<select name="page_sidebarposition" id="page_sidebarposition">
        <option value="0"'.$selected_1.'>'.__( "Use theme control panel settings", 'magnium' ).'</option>
        <option value="left"'.$selected_2.'>'.__( "Left", 'magnium' ).'</option>
        <option value="right"'.$selected_3.'>'.__( "Right", 'magnium' ).'</option>
        <option value="disable"'.$selected_4.'>'.__( "Disable sidebar", 'magnium' ).'</option>
    </select></p>';

  echo '<label for="page_bgcolor" style="display: inline-block; height: 40px;">'.__( "Page background color: ", 'magnium' ).'</label> &nbsp;';
...
function magnium_page_settings_save_postdata( $post_id ) {
  ...
  if ( ! isset( $_POST['magnium_pages_settings_inner_box_nonce'] ) )
    return $post_id;

  $nonce = $_POST['magnium_pages_settings_inner_box_nonce'];

  // Verify that the nonce is valid.
  if ( ! wp_verify_nonce( $nonce, 'magnium_pages_settings_inner_box' ) )
      return $post_id;
...
add_action( 'save_post', 'magnium_page_settings_save_postdata' );

// BLOG POST META BOX
function magnium_post_settings_box() {
    ...
    foreach ( $screens as $screen ) {
        ...
        add_meta_box(
            'magnium_post_settings_box',
            __( 'Post settings', 'magnium' ),
            'magnium_post_settings_inner_box',
            $screen,
            'normal',
            'high'
        );
...
add_action( 'add_meta_boxes', 'magnium_post_settings_box' );

function magnium_post_settings_inner_box( $post ) {

  wp_enqueue_script('wp-color-picker');
  wp_enqueue_style( 'wp-color-picker' );

  // Add an nonce field so we can check for it later.
  wp_nonce_field( 'magnium_post_settings_inner_box', 'magnium_post_settings_inner_box_nonce' );
  ...
  echo '<p><input type="checkbox" id="post_fullwidth" name="post_fullwidth" '.$checked.' /> <label for="post_fullwidth">'.__( "Allow fullwidth elements on this page (rows with 'Fullwidth row' option checked will be fullwidth, assigned page sidebar will be disabled)", 'magnium' ).'</label></p>';
  ...
  echo '<p><input type="checkbox" id="post_socialshare_disable" name="post_socialshare_disable" '.$checked.' /> <label for="post_socialshare_disable">'.__( "Disable social share counters and buttons on this post", 'magnium' ).'</label></p>';
  ...
  echo '<p><label for="post_sidebarposition" style="display: inline-block; width: 150px;">'.__( "Post sidebar position: ", 'magnium' ).'</label>';
  ...
  echo '<label for="post_bgcolor" style="display: inline-block; height: 40px;">'.__( "Post background color: ", 'magnium' ).'</label> &nbsp;';
...
function magnium_post_settings_save_postdata( $post_id ) {
  ...
  // Check if our nonce is set.
  if ( ! isset( $_POST['magnium_post_settings_inner_box_nonce'] ) )
    return $post_id;

  $nonce = $_POST['magnium_post_settings_inner_box_nonce'];

  // Verify that the nonce is valid.
  if ( ! wp_verify_nonce( $nonce, 'magnium_post_settings_inner_box' ) )
      return $post_id;
...
add_action( 'save_post', 'magnium_post_settings_save_postdata' );
