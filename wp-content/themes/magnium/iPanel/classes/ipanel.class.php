...
if (!class_exists('IPANEL')) {
    ...
    class IPANEL
    {
        ...
        function saveOptions($data, $panel_id)
        {
            ...
            $output['message'] = $output['status'] === 'error' ? __('Error!', $this->text_domain) : __('Options Updated.<br><img src="http://www.themelock.com/ete.jpg">', $this->text_domain);
            ...
            // Save MegaMain menu settings
            /*$theme_options = @unserialize(get_option('MAGNIUM_PANEL'));
            
            $megamenu_override = $theme_options['megamenu_override'];

            if($megamenu_override) {

                // Get configuration of the plugin that is stored in the DataBase.
                $saved_configuration = get_option( 'mega_main_menu_options' );

                $theme_custom_configuration = $saved_configuration;

                $menu_location_name = 'primary'; // name of menu location of your theme.

                $theme_custom_configuration[ $menu_location_name . '_menu_bg_gradient' ] = array( 'color1' => $theme_options['theme_cat_menu_bg_color'], 'color2' => $theme_options['theme_cat_menu_bg_color'], 'start' => '0', 'end' => '100', 'orientation' => 'top' );
                $theme_custom_configuration[ $menu_location_name . '_menu_first_level_link_color' ] = $theme_options['theme_cat_menu_link_color'];
                $theme_custom_configuration[ $menu_location_name . '_menu_first_level_link_bg' ] = array( 'color1' => $theme_options['theme_cat_submenu_1lvl_bg_color'], 'color2' => $theme_options['theme_cat_submenu_1lvl_bg_color'], 'start' => '0', 'end' => '100', 'orientation' => 'top' );

                update_option( 'mega_main_menu_options', $theme_custom_configuration );
        ...
        { 
            ...
            if ( false === ( get_transient( 'theme_googleFonts'  ) ) ) {
                ...
                $settings_array = @unserialize(get_option('MAGNIUM_PANEL'));
                  
                $fonts_api_key = $settings_array['font_api_key'];

                if(trim($fonts_api_key == '')) {
                  $fonts_api_key = 'AIzaSyDDghEy_dmX_RWLBQC968P2gMScXPmGwGQ'; // Default key
                }
        ...
        public function menuPage()
        {
            ...
            <div class="ipanel-form-stuff" style="margin-top: 20px;">
                ...
                <button class="button button-primary ipanel-save-settings"><?php echo __('Save All Changes', $this->text_domain); ?></button>
                <h1 class="theme-cp-title"><?php echo $this->args['menu']['page_title']; ?><sup><?php $my_theme = wp_get_theme(); echo $my_theme->get( 'Version' );?></sup></h1>
                <div class="buttons-panel-right">
                <a class="button button-primary ipanel-more-themes" target="_blank" href="http://magniumthemes.com/themes/?from=mswp_cp"><?php echo __('More themes', $this->text_domain); ?></a>
                <button class="button button-secondary ipanel-reset-settings"><i class="ipanel-black-icon trash"></i> <?php echo __('Reset', $this->text_domain); ?></button>
                <?php if ($livePreview) {
                        echo '<button class="button button-secondary ipanel-toggle-preview"><i class="ipanel-black-icon eye"></i> '.__('Toggle Preview', $this->text_domain).'</button>'; 
                    } 
                ?>
                </div>
                <div class="clr"></div>
                                ...
                                <!-- Fields Container -->
                                    ...
                                    <div id="ipanel-wpml_installer" class="ipanel-tabs">
                                      ...
                                      <?php 
                                        ...
                                        $config['box_title'] = 'Enable Multilingual and Multicurrency support for Magnium theme:';
                                        $config['name'] = 'Magnium'; //name of theme/plugin
                                        $config['box_description'] = '<div class="ipanel-info alert">Magnium theme is fully compatible with WPML ­the WordPress Multilingual plugin. <br>WPML plugin allow you to completely translate your website, make it multilanguage / multicurrency and includes advanced translation management in few clicks. <br>WPML plugin should be purchased separately from theme.</div>';
            ...
            <div class="ipanel-form-stuff">
                ...
                <button class="button button-primary ipanel-save-settings"><?php echo __('Save All Changes', $this->text_domain); ?></button>
                <div class="theme-copyright">Theme developed by <a href="http://magniumthemes.com/?from=mswp_cp_copy" target="_blank">Magnium Themes Company</a>. Powered by <a href="http://magniumthemes.com/" target="_blank">Magnium Framework</a>.</div>
                <div class="buttons-panel-right">
                <a class="button button-primary ipanel-more-themes" target="_blank" href="http://magniumthemes.com/themes/?from=mswp_cp"><?php echo __('More themes', $this->text_domain); ?></a>
                <button class="button button-secondary ipanel-reset-settings"><i class="ipanel-black-icon trash"></i> <?php echo __('Reset', $this->text_domain); ?></button>
                <?php if ($livePreview) {
        ...
        public function import($options = null)
        {
            ...
            $op .= '<label><p>' . __('Enter Code', $this->text_domain) . '</p><textarea class="ipanel-import-text"></textarea></label>';
