...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        // update post_id
        // 
        // Patched due to js-error causing media library
        // to break
        try {
...
(function ($) {
    ...
    acf.fields.relationship = {
        ...
        init: function () {
            ...
            // add var to this for if statement.
            if (this.$render === true) {
                this.fetch();
            };
        ...
        render: function (json) {
            ...
            // update classes
            _this.$el.removeClass('no-results').removeClass('loading');
            ...
            // no results?
            if (!json || !json.html) {
                _this.$el.addClass('no-results');
                return;
            }


            // append new results
            _this.$el.find('.relationship_left .load-more').before(json.html);
            ...
            _this.$left.find('a').each(function () {
    ...
    acf.fields.relationship.fetching = {
        allRelations: new Array(),
        fetchAll: function () {
            var fields = [];


            $.each(acf.fields.relationship.fetching.allRelations, function () {
                fields.push(this.o);
            });
            //loop this
            for (var i = acf.fields.relationship.fetching.length - 1; i >= 0; i--) {
                acf.fields.relationship.fetching[i].$el.addClass('loading');
            };
            //$el.addClass('loading');
            var data = {
                action: 'acf/fields/relationship/query_posts_for_all',
                post_id: acf.o.post_id,
                nonce: acf.o.nonce,
                gah: fields,
            };
            ...
            ajax.done(function (json) {
                for (var i = json.length - 1; i >= 0; i--) {
                    ...
                    var currentField = acf.fields.relationship.fetching.allRelations[i];

                    currentField.render(currentJson);
                };
            })
            ajax.fail(function () {
                //alert( "error" );
    ...
    $(document).on('acf/setup_fields', function (e, el) {
        var arr = new Array();
        $(el).find('.acf_relationship').each(function (i) {
            var field = new RelationShipObject();
            field.set({ $el: $(this), $render: false }).init();
            arr.push(field);

        });
        acf.fields.relationship.fetching.allRelations = arr;
        acf.fields.relationship.fetching.fetchAll();
