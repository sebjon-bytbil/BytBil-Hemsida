...
class acf_field_relationship extends acf_field
{
    ...
    */
    ...
    function __construct()
    {
        ...
        //for multiple fields
        add_action('wp_ajax_acf/fields/relationship/query_posts_for_all', array($this, 'query_posts_for_all'));
        add_action('wp_ajax_nopriv_acf/fields/relationship/query_posts_for_all', array($this, 'query_posts_for_all'));
    }
    ...
    */
    ...
    function get_relations($args){
        ...
        //$useCache = false;
        $return = false;
        //if (isset($types[$options['field_key']])) {
            $useCache = true;
            $cacheKey = 'acf_relationship_'.$options['field_key'].get_current_blog_id();
    	    //error_log($cacheKey);
            $return = wp_cache_get( $cacheKey, 'acf_relationship');
        //}
        ...
        if (!$return ) {
            ...
            $return = $r;
            if ($useCache) {
                wp_cache_set( $cacheKey, $return, 'acf_relationship', 0 );
            }
    ...
    */
    ...
    {   
        ...
        foreach ($fields as $key => $field) {
            $field['post_id'] = $_POST['post_id'];
            
            $returnFields[] = $this->get_relations($field);
        }

        

        // return JSON
        echo json_encode($returnFields);
        $end = microtime(true);

        error_log( ($end - $start).' seconds' );
        die();
    ...
    */
    ...
    function query_posts()
    {
        ...
        $useCache = false;
        $return = false;
        if (in_array($options['field_key'], $types)) {
            $useCache = true;
            $cacheKey = 'acf_relationship_'.$options['field_key'].get_current_blog_id();
            $return = wp_cache_get( $cacheKey, 'acf_relationship');
        }
    ...
    */
    ...
    function create_field($field)
    {
        ...
        ?>
        <div
            class="acf_relationship<?php echo $class; ?>"<?php foreach ($attributes as $k => $v): ?> data-<?php echo $k; ?>="<?php echo $v; ?>"<?php endforeach; ?>>
