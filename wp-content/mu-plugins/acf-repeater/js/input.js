    ...
    acf.fields.repeater = {
        ...
        add: function ($before) {
            ...
            var new_id = acf.helpers.uniqid(),
                new_field_html = this.$el.find('> .content-wrapper > table > tbody > tr.row-clone, > table > tbody > tr.row-clone').html().replace(/(=["]*[\w-\[\]]*?)(acfcloneindex)/g, '$1' + new_id),
                new_field = $('<tr class="row"></tr>').append(new_field_html);


            // add row
            if (!$before) {
                $before = this.$el.find('> .content-wrapper > table > tbody > .row-clone, > table > tbody > tr.row-clone');
            }
