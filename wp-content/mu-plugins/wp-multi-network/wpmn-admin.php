    ...
    function admin_head()
    {
        ...
        <script type="text/javascript">
            jQuery(document).ready(function () {
                ...
                jQuery('input[name=assign]').click(function () {
                    move('from', 'to');
                });
                jQuery('input[name=unassign]').click(function () {
                    move('to', 'from');
                });
        ...
        <div class="wrap">
            ...
            <h2><?php esc_html_e('Networks'); ?>
                ...
                <?php if (current_user_can('manage_network_options')) : ?>
                    ...
                    <a href="<?php echo add_query_arg(array('page' => 'add-new-network'), $this->admin_url()); ?>"
            ...
            <form action="<?php echo add_query_arg(array('action' => 'domains'), $this->admin_url()); ?>" method="post"
                  id="domain-search">
                <?php $wp_list_table->search_box(esc_html__('Search Networks'), 'networks'); ?>
                <input type="hidden" name="action" value="domains"/>
            </form>

            <form id="form-domain-list"
                  action="<?php echo add_query_arg(array('action' => 'domains'), $this->admin_url()); ?>" method="post">
                ...
                <form method="POST" action="<?php echo $this->admin_url(); ?>">
                    <table class="form-table">
                        <tr>
                            ...
                            <td><input type="text" name="name" id="newName"
                                       title="<?php esc_html_e('A friendly name for your new network'); ?>"/></td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="newDom"><?php esc_html_e('Domain'); ?>:</label></th>
                            <td> http://<input type="text" name="domain" id="newDom"
                                               title="<?php esc_html_e('The domain for your new network'); ?>"/></td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="newPath"><?php esc_html_e('Path'); ?>:</label></th>
                            <td><input type="text" name="path" id="newPath" value="/"
                                       title="<?php esc_html_e('If you are unsure, put in /'); ?>"/></td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="newSite"><?php esc_html_e('Site Name'); ?>:</label></th>
                            <td><input type="text" name="newSite" id="newSite"
                                       title="<?php esc_html_e('The name for the new network\'s site.'); ?>"/></td>
                        </tr>
                    </table>
                ...
                <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
                                ...
                                <select name="to" id="to">
                                    ...
                                    <?php foreach ($sites as $network) : ?>
                                        <?php if ($network->id != $myNetwork->id) : ?>
                                            <option
                                                value="<?php echo esc_attr($network->id); ?>"><?php echo esc_html($network->domain); ?></option>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                </select>
                            </td>
                    ...
                    <?php if (has_action('add_move_blog_option')) : ?>
                        <table class="widefat">
                            <thead>
                            <tr scope="col">
                                <th colspan="2"><?php esc_html_e('Options'); ?>:</th>
                            </tr>
                            </thead>
                ...
                <form method="post" id="site-assign-form" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
                    ...
                    <noscript>
                        <div id="message" class="updated">
                            <p><?php esc_html_e('Select the blogs you want to assign to this network from the column at left, and click "Update Assignments."'); ?></p>
                        </div>
                    </noscript>
                    <table class="widefat">
                        <thead>
        ...
        if (isset($_POST['update']) && isset($_GET['id'])) {
            ...
            <div class="wrap">
                ...
                <form method="post" action="<?php echo remove_query_arg('action'); ?>">
                    <table class="form-table">
                        <tr class="form-field">
                            <th scope="row"><label for="domain"><?php esc_html_e('Domain'); ?></label></th>
                            <td> http://<input type="text" id="domain" name="domain"
                                               value="<?php echo esc_attr($network->domain); ?>"></td>
                        </tr>
                        <tr class="form-field">
                            <th scope="row"><label for="path"><?php esc_html_e('Path'); ?></label></th>
                            <td><input type="text" id="path" name="path"
                                       value="<?php echo esc_attr($network->path); ?>"/></td>
                        </tr>
                    </table>
    ...
    function delete_network_page()
    {
        ...
        if (isset($_POST['delete']) && isset($_GET['id'])) {
            ...
            <form method="POST" action="<?php echo remove_query_arg('action'); ?>">
                ...
                <h3><?php esc_html_e('Delete Network'); ?>
                    : <?php echo esc_html($network->domain . $network->path); ?></h3>
                    ...
                    if (!empty($sites)) {
                        if (RESCUE_ORPHANED_BLOGS && ENABLE_NETWORK_ZERO) { ?>
                            <div id="message" class="error">
                                ...
                                <p><label
                                        for="override"><?php esc_html_e('If you still want to delete this network, check the following box'); ?>
                ...
                <form method="POST" action="<?php echo $this->admin_url(); ?>">
                    <div>
                        <?php if ($sites) {
                            if (RESCUE_ORPHANED_BLOGS && ENABLE_NETWORK_ZERO) { ?>
                                <div id="message" class="error">
                                    <h3><?php esc_html_e('You have selected the following networks for deletion'); ?>
                                        :</h3>
                                    <ul>
                                        <?php foreach ($network as $deleted_network) { ?>
                                            <li><input type="hidden" name="deleted_networks[]"
                                                       value="<?php echo esc_attr($deleted_network->id); ?>"/><?php echo esc_html($deleted_network->domain . $deleted_network->path); ?>
                                            </li>
                                        <?php } ?>
                                    </ul>
                                    ...
                                    <p><label
                                            for="override"><?php esc_html_e('If you still want to delete these networks, check the following box'); ?>
                                            :</label> <input type="checkbox" name="override" id="override"/></p>
                                </div>
                            <?php } else { ?>
                                <div id="message" class="error">
                        ...
                        <input type="submit" name="delete_multiple" value="<?php esc_html_e('Delete Networks'); ?>"
                               class="button"/>
